...
from backend.retrieval import format_context_for_llm, get_query_embedding, retrieve, retrieve_and_format
from backend.diff_utils import validate_diff
from backend.tools import apply_patch as tool_apply_patch, get_file as tool_get_file, search_symbol
from backend.workspace_utils import get_repo_root
...
async def detect_intent(state: dict) -> dict:
    """Classify user intent: chat | generate | debug."""
...
async def coder(state: dict) -> dict:
    """Produce a unified diff only. Use plan and retrieved context."""
    chunks = state.get("retrieved_chunks") or []
...
async def apply_patch_node(state: dict) -> dict:
    """Call apply_patch tool; update state with changed_files."""
    diff = state.get("diff") or ""
    session_id = state.get("session_id") or ""
